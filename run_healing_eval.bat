@echo off
REM Self-Healing Framework: Complete Evaluation Pipeline (Windows)
REM Run all three diagnostic scenarios for the Stochastic Spectral Stability framework

setlocal enabledelayedexpansion

set DATASET=%1
if "%DATASET%"=="" set DATASET=chameleon

set EPOCHS=%2
if "%EPOCHS%"=="" set EPOCHS=100

set WARMUP=%3
if "%WARMUP%"=="" set WARMUP=20
...
set "COMMON_ARGS=--dataset %DATASET% --epochs %EPOCHS% --warmup-epochs %WARMUP% --add-knn 10 --anchor-knn 10 --alpha-conn 1.0 --conn-eps 0.2 --edge-budget-min-ratio 0.6 --edge-budget-max-ratio 1.4 --dual-lr 0.02 --cvar-samples 10 --cvar-frac 0.5 --shock-samples 200 --shock-cvar-frac 0.2 --shock-reset-duals --seed %SEED%"

REM ============================================================
REM Scenario 1: Is adaptation activating? (Run A)
REM ============================================================
echo.
echo ════════════════════════════════════════════════════════════════
...
python scripts/run_nodecls_wikipedia.py %COMMON_ARGS% --zero-shot-shock --shock-drop-pct 0.5 --shock-adapt --shock-adapt-steps 30 --shock-primal-steps 5 --shock-primal-lr 0.01 --shock-primal-samples 8 --log-csv data/metrics/scenario1_activation.csv

REM ============================================================
REM Scenario 2: Is rewiring the accuracy bottleneck? (Run B)
REM ============================================================
echo.
echo ════════════════════════════════════════════════════════════════
...
python scripts/run_nodecls_wikipedia.py %COMMON_ARGS% --alpha-conn 0.0 --alpha-edge-budget 0.0 --zero-shot-shock --shock-drop-pct 0.1 --log-csv data/metrics/scenario2_no_constraints.csv

REM ============================================================
REM Scenario 3: Targeted Spectral Attack + Comparative Eval
REM ============================================================
echo.
echo ════════════════════════════════════════════════════════════════
...
python scripts/run_nodecls_wikipedia.py %COMMON_ARGS% --zero-shot-shock --targeted-spectral-attack --attack-pct 0.15 --tta-healing-steps 20 --tta-dual-lr 0.01 --comparative-eval --log-csv data/metrics/scenario3_targeted_attack.csv
...
echo 2. Compare Scenario 1: λ₂_post-adapt than λ₂_pre-adapt?
